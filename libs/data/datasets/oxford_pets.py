...
class OxfordPets(DatasetBase):
    ...
    def split_trainval(trainval, p_val=0.2):
        p_trn = 1 - p_val
        logger.info(f"Splitting trainval into {p_trn:.0%} train and {p_val:.0%} val")
    ...
    def save_split(train, val, test, filepath, path_prefix):
        ...
        logger.info(f"Saved split to {filepath}")
    ...
    def read_split(filepath, path_prefix):
        ...
        logger.info(f"Reading split from {filepath}")
    ...
    def subsample_classes(*args, subsample="all"):
        ...
        logger.info(f"SUBSAMPLE {subsample.upper()} CLASSES!")
        if subsample == "base":
